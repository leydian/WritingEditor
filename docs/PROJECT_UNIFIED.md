...
- 캐시 핫픽스
  - `index.html`의 `ui-bindings.js` 버전을 `v=2`로 상향해 구버전 캐시로 인한 토글 미동작 이슈 차단

20. 기록 패널 클릭 불가 핫픽스 (2026-03-01)
- 증상: 기록 패널(뽀모도로/달성기록)이 열리지만 내부 버튼/입력이 클릭되지 않음
- 원인: `.app`의 `z-index`로 stacking context가 생성되어, `#panel-backdrop`이 패널 상호작용을 가로채는 레이어 충돌 발생
- 조치:
  - `styles/layout.css`에서 `.app`의 `z-index` 제거
  - `index.html` 스타일 버전 `styles.css?v=20`으로 상향(캐시 무효화)
- 결과: 기록 패널 오픈 시 내부 컨트롤(타이머 버튼, 달력 전환, 목표 입력) 정상 클릭 가능
